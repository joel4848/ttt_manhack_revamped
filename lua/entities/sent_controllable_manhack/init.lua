        ...
        if self:GetSequence() ~= self:LookupSequence(self.SequenceDeploy) then
            ...
            if IsValid(collisionData.HitEntity) then
                ...
                if collisionData.HitEntity.AddEntityRelationship and collisionData.HitEntity.Disposition then
                    ...
                    local tempManhack = ents.Create("npc_manhack")
...
function ENT:StartControlling()
    ...
    if IsValid(playerController) and self:GetSequence() ~= self:LookupSequence(self.SequenceDeploy) then
            ...
            if ply == playerController then
                ...
                elseif key == IN_USE and ControllableManhack.ConVarAllowInteract() then
                    ...
                    if IsValid(trace.Entity) then
                        if trace.Entity:GetClass() == "ttt_traitor_button" and trace.Entity.TraitorUse then
                            trace.Entity:TraitorUse(playerController)
                        else
...
hook.Add("OnEntityCreated", ControllableManhack.manhackEntityClassName .. ".SetupEntityRelationship", function(ent)
    if ent:GetClass() ~= "npc_manhack" and ent.AddEntityRelationship and ent.Disposition then
        --Used to get default relationships
        local tempManhack = ents.Create("npc_manhack")
